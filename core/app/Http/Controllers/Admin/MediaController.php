...
class MediaController extends Controller
{
    ...
    {
        return Inertia::render('Admin/Media/Libary');
    }
    ...
    public function ckuploadImage(Request $request)
    {
        ...
        // $name = $this->slugString($settings->name);$request->file->extension();
        ...
        $xlPath = $this->getStoragePath('xl') . '/';
        $lgPath = $this->getStoragePath('lg') . '/';
        $mdPath = $this->getStoragePath('md') . '/';
        $smPath = $this->getStoragePath('sm') . '/';

        $originalImage = InterventionImage::make($images)->encode($format, 100);
        ...
        $iii = Storage::put($savePath, $originalImage);

        $this->encodeSave($images, $lgSize, $format, $lgPath, $fileName);
        $this->encodeSave($images, $mdSize, $format, $mdPath, $fileName);
        $this->encodeSave($images, $smSize, $format, $smPath, $fileName);
    ...
    public function uploadImage(Request $request)
    {
        ...
        // {
        //     $this->multiSize($image , $uploadPath , $settings[$index]);
        // }https://laravel.io/images/laravelio-logo.svg
        ...
        return  response()->json(Storage::url($savePath));
    }
